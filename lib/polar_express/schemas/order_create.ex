...
defmodule PolarExpress.Schemas.OrderCreate do
  ...
  @typedoc """
  * `amount` - A custom amount to charge, in the smallest currency unit. Overrides the product's price; defaults to the product's configured price (0 for free products). A positive amount must be at least the currency's minimum. Can't be combined with `units`. Nullable.
  ...
  * `product_id` - The ID of the one-time product to charge for. Must belong to the order's organization. Only fixed-price, free and unit-based products are supported. Format: uuid4.
  * `units` - The number of units to charge for. Required when the product has unit-based pricing, and rejected otherwise. The amount comes from the price's tiers. Can't be combined with `amount`. Nullable.
  """
  @type t :: %__MODULE__{}
