defmodule PolarExpress.Generator.ServiceGenerator do
  @moduledoc false
  ...
  def generate(spec) do
    ...
    service_files =
      Enum.flat_map(services, &generate_service(&1, path_specs, resource_docs, schema_index))

    namespace_files = generate_namespace_services(services)
    aggregate_files = generate_aggregates(services)
    service_files ++ namespace_files ++ aggregate_files
  end
  ...
  defp generate_service(service, path_specs, resource_docs, schema_index) do
    ...
    methods =
      service.operations
      |> Enum.map_join("\n", fn op ->
        generate_method(op, op.method_name in conflicting_methods, path_specs, schema_index)
      end)
  ...
  defp build_spec_attr(op, resolved_schema, schema_index) do
    ...
    is_list_endpoint =
      case op.response_schema do
        nil -> false
        ref -> OpenAPI.list_response_schema?(ref, schema_index)
      end
  ...
  defp build_merge_opts(op, resolved_schema) do
    extra_opts = []
    ...
    extra_opts =
      if resolved_schema do
        extra_opts ++ ["resource: PolarExpress.Schemas.#{resolved_schema}"]
      else
        extra_opts
      end
  ...
  defp generate_aggregates(services) do
    ...
    v1_file = generate_aggregate_module(PolarExpress.Services.V1, v1_services)
    v2_file = generate_aggregate_module(PolarExpress.Services.V2, v2_services)
  ...
  defp generate_aggregate_module(module, services) do
    ...
    label =
      case module do
        PolarExpress.Services.V1 -> "V1"
        PolarExpress.Services.V2 -> "V2"
      end
  ...
  defp aggregate_packages(PolarExpress.Services.V1, services) do
    services
    |> Enum.map(& &1.package)
    |> Enum.reject(&(&1 == ""))
  ...
  defp aggregate_packages(PolarExpress.Services.V2, services) do
    services
    |> Enum.map(& &1.package)
    |> Enum.reject(&(&1 == ""))
  ...
  # -- Params cross-reference --------------------------------------------------

  defp build_params_module_ref(op) do
    class_name = op.service_class
    package = op.package || ""
    method_name = op.method_name

    if class_name && method_name do
      module = Naming.params_module(class_name, package, method_name)
      inspect(module)
    else
      nil
    end
