defmodule PolarExpress.Generator.ParamsGenerator do
  @moduledoc false
  ...
  defp resolve_param_type(name, %{"anyOf" => variants}) do
    ...
    case non_null do
      ...
      multiple when length(multiple) >= 2 ->
        resolve_multi_variant_param(name, multiple, has_null)
  ...
  defp resolve_multi_variant_param(name, variants, has_null) do
    resolved_base =
      variants
      |> Enum.map(fn variant ->
        {t, _} = resolve_param_type(name, variant)
        base_type(t)
      end)
      |> Enum.uniq()

    type =
      case resolved_base do
        [single_type] -> single_type
        _ -> :map
      end

    if has_null, do: {{:nullable, type}, %{}}, else: {type, %{}}
  end
  ...
  # Extract the base type from possibly-wrapped types
  defp base_type({:nullable, inner}), do: base_type(inner)
  defp base_type({:list, _} = t), do: t
  defp base_type({:nested, _} = t), do: t
  defp base_type(t), do: t
