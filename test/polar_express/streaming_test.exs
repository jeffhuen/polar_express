defmodule PolarExpress.StreamingTest do
  ...
  describe "raw_request/4" do
    ...
    test "returns error tuple on connection failure", %{client: client} do
      ...
      bad_client = %{client | base_url: "http://localhost:1", max_retries: 0}

      {:error, error} = Client.raw_request(bad_client, :get, "/v1/customers/")

      assert %PolarExpress.Error{type: :api_connection_error} = error
    end
